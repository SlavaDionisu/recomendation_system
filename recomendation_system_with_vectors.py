...
def load_models():
    model_path = get_model_path("./catboost_model_with_vectors")
    from_file = CatBoostClassifier() 
    return from_file.load_model(model_path)

def batch_load_sql(query: str, params: dict = None) -> pd.DataFrame: # штука что загружать из бд чанками быстрее
    CHUNKSIZE = 200000
    engine = create_engine(DB_URL)
    conn = engine.connect().execution_options(stream_results=True)
    chunks = []
    query = text(query)
    for chunk_dataframe in pd.read_sql(query, conn, params=params, chunksize=CHUNKSIZE):
...
def load_features() -> pd.DataFrame:
    ...
    df_feeds = batch_load_sql('SELECT post_id, user_id FROM public.feed_data')
    return df_users, df_posts, df_feeds

df_users, df_posts, df_feeds  = load_features()
